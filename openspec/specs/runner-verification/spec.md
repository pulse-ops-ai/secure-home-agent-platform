...
deterministic conformance failure — never a convention. The guard SHALL be
**historically append-only**: the proposed identity record is compared
against the trusted accepted record from the merge base, and an accepted
entry may neither change nor disappear — only new identity@version entries
may append. Rewriting an accepted entry SHALL fail regardless of whether
the regenerated bytes match the rewritten digest. Compatibility
...
#### Scenario: Rewriting an accepted ledger entry fails

- **GIVEN** an accepted base recording `foo@1.0.0 → digest-A`
- **AND** a proposed change that alters the shape, keeps the version, and
  rewrites the entry to `foo@1.0.0 → digest-B`
- **WHEN** the historical append-only comparison runs against the trusted
  accepted base
- **THEN** it fails naming `foo@1.0.0` — even though the regenerated bytes
  match the rewritten digest
