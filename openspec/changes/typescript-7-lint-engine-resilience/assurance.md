...
| INV-TS7-29 | Candidate tools execute only in an isolated untrusted subject domain with no secret, `GITHUB_TOKEN`, persisted credential, Docker socket, shared writable cache, or write access to any trusted workspace; **and** the candidate process is separated from the trusted host-side launcher by an explicit OS-level boundary, so the launcher's timeout state, exit-code capture, artifact hashes, and result envelope are unreachable by the process being measured. Candidate output crosses back only as untrusted data. Trust-critical. | security / trust | REQ-SC-007, REQ-SC-008, D14/D15 | PROP-SUBJECT-001, ADV-MAINT-003, MUT-MAINT-007, MUT-MAINT-009 |
...
| candidate attacks the trusted launcher sharing its runner (timers, exit-code capture, result envelope) | launcher-isolation boundary denies the action; a same-UID topology is refused rather than admitted on boundary 1 alone | INV-TS7-29, MUT-MAINT-009 |
...
| PROP-SUBJECT-001 | the untrusted subject domain provides none of {secret, `GITHUB_TOKEN`, persisted credential, Docker socket, shared writable cache, trusted-workspace write} and (if containerized) all enumerated container controls; **and** the candidate never shares the launcher's UID and filesystem context; a missing control or an absent launcher boundary refuses the run | PR-B onward | workflow/container-option structural assertion + launcher-topology assertion + negative test |
...
| MUT-MAINT-009 | run the candidate binary directly under the trusted launcher's own UID and filesystem context — boundary 1 fully satisfied, boundary 2 absent — and have the candidate alter launcher/result-envelope state | launcher-isolation contract test refuses the topology and names the missing boundary; a fresh secret-free runner alone does not admit it | PR-B onward |
