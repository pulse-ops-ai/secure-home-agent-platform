...
1. `replacement-authority-parity` — establish the policy authority, dual-engine
   ...
   candidate-independent default-branch verifier boundary, bounded TS6 API seam,
   and native platform proof while TypeScript 6 and ESLint remain. This scope
...
review's `FOCUSED_CLOSURE_REQUIRED` verdict over commit `aaa0aaf...`; it remains
intentionally historical/current-gate-negative while the planning bytes include
later predecessor-data and trusted-verifier corrections. A fresh independent
epoch-1 review of the final exact head is still required. Neither this directory
nor Proposed ADR-0022 creates implementation authority.
