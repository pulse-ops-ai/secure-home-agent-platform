...
- **Decision:** every contract carries a stable `contract_id` and an
  **exact** `contract_version` (semantic revision, e.g. `1.0.0`).
  ...
  its generated schema bytes. The guard has two layers, and both are
  required. **Current-state consistency:** the conformance suite fails
  ...
  generated identity has no ledger entry. **Historical append-only:** the
  proposed ledger is compared against the **trusted accepted ledger from
  the merge base** — an entry present in the accepted base may neither
  change nor disappear; only new identity@version entries may append. A
  rewritten row therefore fails even when the regenerated bytes match the
  rewritten digest — self-consistency can never launder an identity
  change. (The ledger is authored, never regenerated — regeneration must
  not be able to heal an identity violation.) This first landing
  establishes the genesis entries, for which the historical comparison is
  vacuous; from the next contract change onward, every accepted mapping
  is immutable. **Compatibility posture:** an older strict reader is never
