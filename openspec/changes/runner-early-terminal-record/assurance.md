...
| ET-INV-05 × provenance | a caller could pass an agent principal as the requester and no contract could tell — the shapes are identical | **not claimed structurally.** L4 populates the requester from the run request; for these runs no profile ever resolved, so no profile-derived principal exists to pass. Assigned to L4 (#27) and named in § Traceability |
...
| Requester **provenance** (populated from the request, never from a profile) | **L4** (#27) | lifecycle fixtures there — **deferred, named**: `Principal` validates shape, not origin, so no contract-level proof can exist |
...
obligation is assigned to L4 — where, for these runs, no profile ever
resolved, so no profile-derived principal exists to confuse it with.
(2) The
