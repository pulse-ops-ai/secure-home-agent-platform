...
|---|---|---|
| ET-INV-05 × ET-INV-01 | the requester is an identity, and an identity field is exactly what an authority-laundering path would want | ET-EX-05 plus ET-EX-01/ET-PROP-02: the requester carries no digest and no authority-bearing key is admissible beside it |
...
| ET-PROP-02 | For any key drawn from the evidence bundle's field names, presenting it on an otherwise valid record refuses — strict posture holds across the whole authority-bearing key space, not only the five names ET-EX-01 enumerates |
...
| ET-ADV-06 | `requested_profile` carries a digest, or a profile-derived agent principal is passed as the requester | refuses — the reference has no digest field, and the requester shape admits no authority |
...
| Requester never reads as authority | 1.1 | ET-EX-05, ET-PROP-02, ET-ADV-04/06 |
...
lacking proof:** none. **Recorded limitation, not a gap:** the
