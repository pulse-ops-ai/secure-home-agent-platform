...
| Path policy | repository-declared policy bytes | `captureAuthority` | `Digest` over captured bytes | source yes, snapshot no | **none** — protected | parse → `PathPolicy` v2 (typed rules); component-prefix matching (D8) | materialization decisions; recorded as `identities.path_policy` (`PathPolicyAuthorityIdentity` — mislabeling unrepresentable; landed via #65/#66); verifier compares against its own capture |
| Gate registry | repository-declared registry bytes | `captureAuthority` | `Digest` over captured bytes | source yes, snapshot no | **none** — protected | parse → `GateRegistry` | eligibility (declared-identity check); recorded as `identities.gate_registry` (`GateRegistryAuthorityIdentity` — mislabeling unrepresentable); verifier compares against its own capture |
