...
| RO-INV-90 | The deadline that wins at a boundary carries its instant AND its provenance as one typed value; every stamp is a projection of it, so a governed deadline that wins a minimum is refused as the run's timeout and an attempt ceiling that wins is refused as the attempt's bound — two independently derived facts that could disagree are unrepresentable | trust |
