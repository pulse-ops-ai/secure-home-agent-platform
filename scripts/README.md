...
| [`governance/genesis/`](governance/genesis/README.md) | Pre-activation extraction from exact post-bridge S, retaining PR-2A archive-stage M, and four-class candidate freshness at an explicit revision. Prints candidate/proof material only; no canonical registry, activation, or owner attestation. The shared model still owns every semantic rule |
