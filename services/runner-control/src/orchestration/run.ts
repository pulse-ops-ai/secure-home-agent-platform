/**
 * The run engine: one run's walk through the declared machine.
 *
 ...
 * returned (design D8).
 *
 * Two shapes a run can end in, and only two: a sealed L2 evidence bundle
 * for any run that reached `PROFILE_RESOLVED`, and an early-terminal
 * refusal record for a run that terminated in `REQUESTED` before
 * authority completed. A fabricated bundle is not a third shape — the
...
import { INTERRUPT_TERMINAL, RunDeadline, type InterruptReason } from './deadline.js'
import type { FinishPartial, RunConclusion, RunRequest, RunSignals, Stop } from './result.js'
import { stop } from './result.js'
...
export class Runner {
  ...
  async #walk(request: RunRequest, signals: RunSignals, scope: RunScope): Promise<RunConclusion> {
    ...
    const terminateEarly = async (
      ...
      if (!reached.ok) {
        return stop(await conclude('none', `${reached.detail}: ${detail}`))
      }
      const record = buildEarlyTerminationRecord({
        run_id: request.run_id,
        requester: request.requester,
    ...
    // Cancellation, the deadline, and the race that makes both
    // effective — one owner, so the timers cannot outlive the run.
    const deadline = new RunDeadline(scope, signals.interrupt)
    const aborter = { signal: deadline.signal }
    const untilAborted = <T>(work: Promise<T>): Promise<T | undefined> => deadline.until(work)
    ...
    const abortRun = async (partial: FinishPartial = {}): Promise<PhaseCommand<RunConclusion>> => {
      const reason = deadline.reason ?? 'cancel'
    ...
    const interrupted = (): InterruptReason | undefined => deadline.interrupted()
    const interruptTerminal = INTERRUPT_TERMINAL
    ...
    const openSession = async (): Promise<PhaseCommand<RunConclusion>> => {
      ...
      deadline.arm(deadlineMs, signals.cancelAfterMs)
