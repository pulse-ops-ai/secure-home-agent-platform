# agents/adapters/frameworks/pydantic-ai

Adapter for **PydanticAI**, in the **household** runner class.

> **Status: not implemented.** This directory holds only this README. PydanticAI
> is **not** a dependency of this repository and must not be added until an
> accepted task contract authorizes it.

## Scope

Wraps PydanticAI so a household agent built on it can be launched from an
execution profile and produce the platform's standard event and evidence
contract.

Paired derived image: `secure-home-runner-pydantic-ai` — the base runner plus
this one framework, pinned.

## What belongs here

- Invocation mapping: run request → framework invocation.
- Output mapping: framework output → the shared event and evidence contract,
  including tool-call events.
- Cancellation and failure handling.
- Notes on what the framework cannot express through the neutral contract.

## What does not belong here

- **Credentials or API keys.** Provisioned by the substrate from the profile.
- **Model identifiers.** A profile field, not an adapter constant.
- **Sandbox construction, mounts, network policy, limits.**
- **Agent logic** — [`../../../implementations/`](../../../implementations/).
- **Tool definitions** — the governed tool-surface package (not yet created).
  The framework's tool mechanism must be wired to the **governed** tool surface,
  never to an ad-hoc client.

## Boundary rules

- Cannot widen its sandbox; cannot reach around the substrate.
- **Framework tool calls are platform tool calls.** Every one re-enters through a
  governed enforcement point. A framework-native tool that talks directly to a
  device or database is a bypass.
- Model access is bounded by the profile's routing class, enforced by the
  substrate.
- Emits exactly the same contract as every other adapter.

## Governed by

[`../../README.md`](../../README.md) → [`../../../AGENTS.md`](../../../AGENTS.md) ·
ADRs
[0003](../../../../docs/decisions/ADR-0003-use-framework-neutral-runner-profiles.md),
[0004](../../../../docs/decisions/ADR-0004-treat-agents-as-clients.md),
[0007](../../../../docs/decisions/ADR-0007-route-local-remote-and-cloud-execution-explicitly.md)

## Validation

Future: framework-conformance coverage, plus a check that no framework-native
tool bypasses the governed tool surface.
